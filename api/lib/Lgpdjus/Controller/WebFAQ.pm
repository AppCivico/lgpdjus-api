...
sub webfaq_index {
    my $c    = shift;
    $c->stash(
        texto_faq_index => $c->schema->resultset('Configuraco')->get_column('texto_faq_index')->next()
    );

    my @faqs = $c->schema->resultset('FaqTelaSobre')->search(
        {
            'me.status'             => 'published',
...
sub webfaq_detail {
    ...
    if ($id =~ /^\d+$/) {
        $c->stash(
            faq => $c->schema->resultset('FaqTelaSobre')->search(
                {
                    'me.status' => 'published',
                    'me.id'     => $id,
                },
...
sub webfaq_botao_contato {
    ...
    $c->stash(
        texto_faq_contato => $c->schema->resultset('Configuraco')->get_column('texto_faq_contato')->next()
    );
