package Lgpdjus::Controller::ResetPassword;
...
use Lgpdjus::Logger;
use Lgpdjus::Utils qw/random_string random_string_from check_password_or_die/;
use JSON;
...
sub request_new {
    ...
    my $schema = $c->schema;
    my $found
      = $c->schema->resultset('Cliente')->search({'email' => $email}, {columns => ['id', 'cpf', 'nome_completo']})
      ->next;
    if (!$found) {
        die {
    ...
    my $item = $c->schema->resultset('ClientesResetPassword')->search(
        {
    ...
    $item = $c->schema->resultset('ClientesResetPassword')->create(
        {
    ...
    my $email_db = $c->schema->resultset('EmaildbQueue')->create(
        {
            ...
            to        => $email,
            subject   => 'LGPDjus - Recuperação de senha',
            variables => to_json(
                {
...
sub write_new {
    ...
    # procura o cliente pelo email
    my $schema = $c->schema;
    my $found  = $c->schema->resultset('Cliente')->search({'email' => $email})->next;
    if (!$found) {
        goto INVALID_TOKEN;
    }
    my $directus_id = $found->id;
