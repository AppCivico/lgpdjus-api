...
use utf8;
use Lgpdjus::Test;
use Lgpdjus::Minion::Tasks::NewsIndexer;

my $t = test_instance;

$ENV{MAINTENANCE_SECRET} = '12345';

my $feed_rs = app->schema->resultset('RssFeed');
my $news_rs = app->schema->resultset('Noticia');
my $tags_rs = app->schema->resultset('Tag');
my $rules_rs = app->schema->resultset('TagIndexingConfig');

my $base = 'https://lgpdjus-api.appcivico.com/.tests-assets';
...
do {
    ...
    ok(Lgpdjus::Minion::Tasks::NewsIndexer::news_indexer($job, test_get_minion_args_job(0)), 'indexing news 0');
    $minion_job_id++;
    ...
    ok(Lgpdjus::Minion::Tasks::NewsIndexer::news_indexer($job, test_get_minion_args_job(1)), 'indexing news 1');
    $minion_job_id++;
    ...
    do {
        ...
        is test_get_minion_args_job($minion_job_id+1), [$news2[0]->id], 'page2 id is minion job 3';

        trace_popall;
        ok(
            Lgpdjus::Minion::Tasks::NewsIndexer::news_indexer($job, test_get_minion_args_job($minion_job_id + 1)),
            'indexing...'
        );
        $minion_job_id++;
    ...
    my ($session, $user_id) = get_user_session($random_cpf, 'rss');
    ...
    $t->get_ok(
        ('/news-timeline'),
        {'x-api-key' => $session}
    )->status_is(200)->json_is('/rows/0/type', 'news_group')
      ->json_is('/rows/0/news/0/title', 'This is Page1 Title')->json_is('/has_more', '0')->tx->res->json;

    # testa visao noticias, mas passando tags
    # deve trazer noticias ou tweets marcados
    $t->get_ok(
        ('/news-timeline?tags=' . $tag1->id),
        {'x-api-key' => $session}
    )->status_is(200)->json_is('/rows/0/type', 'news_group')
      ->json_is('/rows/0/news/0/title', '“Mulher” teste decode encoded titles')->json_is('/has_more', '0')
      ->tx->res->json;
...
sub clean_up {
    ...
    app->schema->resultset('NoticiasAbertura')
